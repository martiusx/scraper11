                          ...
                          script.onload = async function() {
                              if (typeof opis === 'function') {
                                  if (window.location.href.includes('wyszukiwarka-s') || window.location.href.includes('search') || window.location.href.includes('global-search?')) {
                                                ...
                                                body: JSON.stringify({
                                                    ...
                                                    "messages": [
                                                        {"role": "user", 
                                                            ...
                                                            "content": 
                                                                \`odpowiedź powinna być do wstawienia jako kod HTML oraz mieć formę:

                                                                    [nagłówek h3 z tytułem produktu - popraw stylistykę]

                                                                    [Krótki opis z pogrubieniami na słowa kluczowe do pozycjonowania SEO na około 600 znaków]

                                                                    [Tabela z danymi technicznymi- tej nie skracaj tylko wypisz wszystkie dane]

                                                                    Na podstawie tego tekstu:\` + trescOpisu.maintext} // Tutaj przekazujesz zmienną
                                                    ],
                                                })
                                            }).then(response => {
                                                return response.json();
                                            }).then(data => {
                                                trescOpisu.maintext += "----------- Opis z AI --------";
                                                trescOpisu.maintext += data.choices[0].message.content;
                                                saveDataToLocalStorage(trescOpisu.maintext);
                                            }).catch(error => {
                                      ...
                                      console.log(trescOpisu.maintext);
                                      console.log(trescOpisuPoPrzerobieniu + ' ----------------------------');
